...
(hx/defnc ErrorText [{:keys [text]}]
  [:div {:style {:fontSize "0.7em"
                 :display "flex"
                 :align-items "center"
                 :color "red"}} text])

(hx/defnc ExpandButton [{:keys [expanded path emit-fn]}]
...
(hx/defnc ExpandAllButton [{:keys [emit-fn data]}]
...
(hx/defnc CollapseAllButton [{:keys [emit-fn data]}]
...
(hx/defnc CopyButton [{:keys [emit-fn data]}]
...
(hx/defnc StringText [{:keys [data] }]
  [:span {:style (:strings styles)} (pr-str data)])

(hx/defnc KeywordText [{:keys [data]}]
  [:span {:style (:keywords styles)} (str data)])

(hx/defnc NumberText [{:keys [data]}]
  [:span {:style (:numbers styles)} data])

(hx/defnc KeySet [{:keys [keyset]}]
  [:span
   (->> keyset
        (sort-by str)
        (map-indexed
          (fn [i data] ^{:key i} [:span
                                  (cond (nil? data) [NilText]
                                        (string? data) [StringText {:data data }]
                                        (keyword? data) [KeywordText {:data data }]
                                        (number? data) [NumberText {:data data }]
                                        :else (str data))]))
        (interpose " "))])

(hx/defnc Node [{:keys [data path emit-fn swappable metadata-paths]}]
  [:div {:style {:display "flex"}} (cond
   ...
   (when-let [errors (:error (get metadata-paths path))]
     [ErrorText {:text (str "\u00A0 " errors) }])])

(defn expandable? [v]
  (or (map? v) (seq? v) (coll? v)))

(hx/defnc CollectionSummary [{:keys [data]}]
  (cond (map? data) [:div {:style {:flex "0 1 auto"}}
                     [:span "{"]
                     [KeySet {:keyset (keys data) }]
                     [:span "}"]]
...
(hx/defnc KeyValNode [{[k v] :data :keys [path metadata-paths emit-fn swappable]}]
...
(hx/defnc ListVecNode [{:keys [data path metadata-paths emit-fn swappable hide-header]}]
...
(hx/defnc SetNode [{:keys [data path metadata-paths emit-fn swappable hide-header]}]
  (let [metadata (get metadata-paths path)
        expanded? (:expanded? metadata)]
    [:div {:style {:display "flex"
                   :flex-flow "column"}}
...
(hx/defnc MapNode [{:keys [data path metadata-paths emit-fn hide-header] :as all}]
  (let [metadata (get metadata-paths path)
        expanded? (:expanded? metadata)]
    [:div {:style {:display "flex"
                   :flex-flow "column"}}
...
(hx/defnc DataFrisk [{:keys [data] :as all}]
...
(hx/defnc Root [{:keys [ data id state-atom ] }]
  (let [data-frisk (:data-frisk @state-atom)
    ...
    [:div
     [:div {:style {:padding "4px 2px"}}
      [ExpandAllButton {:emit-fn emit-fn :data data}]
      [CollapseAllButton {:emit-fn emit-fn }]
      [CopyButton {:emit-fn emit-fn :data data }]]
     [:div {:style {:flex "0 1 auto"}}
      [DataFrisk {:data data
                  :swappable swappable
                  :path []
...
(hx/defnc VisibilityButton
  [{:keys [ visible update-fn ] }]
  [:button {:style {:border 0
